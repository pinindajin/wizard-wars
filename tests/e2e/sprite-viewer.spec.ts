...
test.describe("sprite viewer dev route", () => {
  ...
  test("smoke: gallery, detail canvas, legend", async ({ page }) => {
    ...
    await expect(page.getByTestId("sprite-viewer-hero-helena")).toBeVisible()
    await page.getByTestId("sprite-viewer-hero-helena").click()
    await expect(page.getByTestId("sprite-viewer-gallery")).toBeVisible()
  ...
  test("overlay toggles remain interactive", async ({ page }) => {
    ...
    const hurtbox = page.getByTestId("sprite-viewer-hurtbox-toggle")
    await expect(collision).toBeVisible()
    await expect(edge).toBeVisible()
    await expect(hurtbox).toBeVisible()
    await collision.uncheck()
    await edge.uncheck()
    await hurtbox.uncheck()
    await expect(collision).not.toBeChecked()
    await expect(edge).not.toBeChecked()
    await expect(hurtbox).not.toBeChecked()
    await collision.check()
    await edge.check()
    await hurtbox.check()
    await expect(collision).toBeChecked()
    await expect(edge).toBeChecked()
    await expect(hurtbox).toBeChecked()
    await expect(page.getByTestId("sprite-viewer-detail-canvas")).toBeVisible()
  })

  test("primary-attack hurtbox legend is documented", async ({ page }) => {
    await page.goto("/dev/sprite-viewer")
    await expect(page.getByTestId("sprite-viewer-legend")).toBeVisible()
    const row = page.getByTestId("sprite-viewer-legend-info-hurtbox-attack")
    await expect(row).toBeVisible()
    await row.click()
    const legend = page.getByTestId("sprite-viewer-legend")
    await expect(legend.getByText(/half-circle/i)).toBeVisible()
    await expect(legend.getByText(/dangerous-frames window/i)).toBeVisible()
  })
})
