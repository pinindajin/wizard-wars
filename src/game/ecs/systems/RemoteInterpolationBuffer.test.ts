...
describe("RemoteInterpolationBuffer", () => {
  ...
  it("clamps to the oldest sample when requested time is in the past", () => {
    ...
    expect(b.sampleAt(1, 50)).toEqual({ x: 10, y: 10, facingAngle: 0, moveFacingAngle: 0 })
  })
  ...
  it("remove drops the entity's samples and clears empties everything", () => {
    const b = new RemoteInterpolationBuffer()
    b.push(1, { serverTimeMs: 0, x: 0, y: 0, vx: 0, vy: 0, facingAngle: 0, moveFacingAngle: 0 })
    b.push(2, { serverTimeMs: 0, x: 0, y: 0, vx: 0, vy: 0, facingAngle: 0, moveFacingAngle: 0 })
    b.remove(1)
