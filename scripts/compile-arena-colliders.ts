...
/**
 * Builds the generated collider source for one object layer.
 *
 * `NonWalkableAreas` is emitted as an exact simplified cover because runtime
 * world collision only needs the union of lava and cliff blockers.
 *
 * @param layerName - Source Tiled object layer.
 * @param exportName - Generated TypeScript export name.
 * @param rects - Layer rectangles.
 * @returns TypeScript source text.
 */
export function buildColliderSource(
  layerName: string,
  exportName: string,
...
/* v8 ignore start -- CLI file IO is covered through pure source-builder tests. */
function main(): void {
    ...
    const body =
      output.layerName === "LavaAreas"
        ...
        : output.layerName === "CliffAreas"
          ...
          : buildColliderSource(output.layerName, output.exportName, rects)
 ...
 */
export function isCompileArenaCollidersCliEntrypoint(
...
/* v8 ignore next 3 */
if (isCompileArenaCollidersCliEntrypoint(process.argv, import.meta.url)) {
  main()
}
