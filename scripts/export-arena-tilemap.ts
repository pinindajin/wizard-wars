...
function assertNativeScene(scene: ArenaScene): void {
  ...
  if (scene.settings?.exportClass !== false) {
    throw new Error("Arena.scene must stay data-only; runtime Arena.ts is generated by build-native-arena-map.ts.")
  }
...
export function buildArenaTilemapFromScene(scene = readArenaScene()): TiledMap {
  ...
  const propObjects = readSceneRectangleObjects(scene, "propCollider_", "prop-collider", nextObjectId)
  nextObjectId += propObjects.length
  const nonWalkableObjects = readSceneRectangleObjects(scene, "nonWalkableArea_", "non-walkable-area", nextObjectId)
  nextObjectId += nonWalkableObjects.length
  const lavaObjects = readSceneRectangleObjects(scene, "lavaArea_", "lava-area", nextObjectId)
  nextObjectId += lavaObjects.length
  const cliffObjects = readSceneRectangleObjects(scene, "cliffArea_", "cliff-area", nextObjectId)
  nextObjectId += cliffObjects.length
  const walkableObjects = readSceneRectangleObjects(scene, "walkableArea_", "walkable-area", nextObjectId)
...
/**
 * Builds generated collider source from one tilemap object layer.
 *
 * `NonWalkableAreas` is emitted as an exact simplified cover for runtime
 * collision while the editor tilemap keeps its original rectangle objects.
 *
 * @param tilemap - Tiled map object.
 * @param layerName - Source object layer name.
 * @param exportName - Generated TypeScript export name.
 * @returns TypeScript source text.
 */
export function buildGeneratedCollidersSource(
  tilemap: TiledMap,
  layerName: string,
  exportName: string,
): string {
  const rects = layerRects(tilemap, layerName)
  const emittedRects = layerName === "NonWalkableAreas" ? simplifyRectCover(rects) : rects
  return `/**
 ...
 */
export const ${exportName} = ${JSON.stringify(emittedRects, null, 2)} as const
`
}
